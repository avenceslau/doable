...
import { createBetterResultBridge, createBetterResultCalleeCodec, createBetterResultCallerCodec, createMetricsFromCodec, createMetricsWithBetterResult, RetryableNetworkError, UserInputError, } from "./examples/better-result-bridge";
import { createContextObservabilityCallee, createContextObservabilityCaller, createFullObservability, createObservabilityCallee, createObservabilityCaller, } from "./examples/observability";
import { createResultBridge, createResultCalleeCodec, createResultCallerCodec, } from "./examples/result-bridge";
import { createRetry } from "./examples/retry";
function createAppDO(options) {
    ...
    const obj = {
        ...
        async createTodo(title) {
            ...
            const item = { id: `todo-${nextId++}`, title };
            obj.items.push(item);
            return item;
        },
        async getTodo(id) {
        ...
        async failNetwork() {
            return Result.err(new RetryableNetworkError({ message: "upstream timeout" }));
        },
        async plainMethod() {
            return "not a Result";
        },
...
describe("retry transform (e2e)", () => {
    it("retries transient failures and succeeds", async () => {
        const ns = withTransforms(mockNamespace(() => createAppDO({ failCount: 2 })));
        const stub = ns.get("x").with(createRetry(3));
        const todo = await stub.createTodo("Buy milk");
        expect(todo.title).toBe("Buy milk");
    });
    it("gives up after retries are exhausted", async () => {
        const ns = withTransforms(mockNamespace(() => createAppDO({ failCount: 5 })));
        const stub = ns.get("x").with(createRetry(2));
        await expect(stub.createTodo("Buy milk")).rejects.toThrowError("transient-error");
    });
    it("passes through on first success without retrying", async () => {
        ...
        baseStub.createTodo = async (title) => {
            callCount();
    ...
    it("accepts RetryOptions object with exponential backoff config", async () => {
        const ns = withTransforms(mockNamespace(() => createAppDO({ failCount: 1 })));
        const stub = ns
            .get("x")
            .with(createRetry({ retries: 2, baseDelay: 1, exponential: true }));
...
describe("observability transform (e2e)", () => {
    describe("caller-side timing", () => {
        it("emits rpc_ok metric on success", async () => {
            ...
            const stub = ns.get("x").with(createObservabilityCaller({
    ...
    describe("callee-side timing", () => {
        it("emits method_ok metric on success", async () => {
            ...
            const wrapped = withCalleeTransforms(instance, [
                createObservabilityCallee((m) => metrics.push(m)),
            ]);
    ...
    describe("context-based observability (PRD pattern)", () => {
        it("propagates accountId/coloId from caller to callee via context", async () => {
            ...
            const instance = createAppDO();
            const wrapped = withCalleeTransforms(instance, [
                createContextObservabilityCallee((m) => calleeMetrics.push(m)),
            ]);
            const stub = withTransforms(mockNamespace(() => wrapped))
                .get("x")
                .with(createContextObservabilityCaller({
                accountId: "acct-42",
                coloId: "DFW",
...
describe("result bridge transform (e2e)", () => {
    describe("separate caller + callee codecs", () => {
        it("wraps success value in envelope and unwraps on caller side", async () => {
            const calleeCodec = createResultCalleeCodec();
            const callerCodec = createResultCallerCodec();
            const instance = createAppDO();
        ...
        it("wraps errors in envelope and re-throws on caller side", async () => {
            ...
            const stub = withTransforms(mockNamespace(() => wrappedInstance))
                .get("x")
                .with(callerCodec);
            const error = await stub.failingMethod().catch((e) => e);
    ...
    describe("full bridge (caller().callee())", () => {
        it("round-trips a success value through the bridge", async () => {
            const bridge = createResultBridge();
            const instance = createAppDO();
        ...
        it("round-trips an error through the bridge", async () => {
            ...
            const stub = withTransforms(mockNamespace(() => wrappedInstance))
                .get("x")
                .with(bridge);
            await expect(stub.failingMethod()).rejects.toThrowError("permanent-failure");
        });
    });
});
// ===========================================================================
...
describe("auth guard transform (e2e)", () => {
    ...
    it("blocks calls with invalid token", async () => {
        ...
        await expect(wrapped.createTodo("Hacker attempt", "wrong-token")).rejects.toThrow(UnauthorizedError);
    });
    it("blocks calls with missing token", async () => {
        ...
        await expect(wrapped.createTodo("No token")).rejects.toThrow(UnauthorizedError);
    });
    ...
    it("guards all methods when protectedMethods is omitted", async () => {
        const guard = createAuthGuard({
            tokenExtractor: (args) => args.find((a) => typeof a === "string" && a.startsWith("s3")),
        });
        const instance = createAppDO();
        const wrapped = withCalleeTransforms(instance, [guard], {
            env: fakeEnv,
...
describe("composing multiple transforms (e2e)", () => {
    it("retry + observability on caller side", async () => {
        ...
        const stub = ns
            .get("x")
            .with(createObservabilityCaller({
            emitMetric: (m) => metrics.push(m),
        }))
            .with(createRetry(2));
    ...
    it("auth guard + observability on callee side", async () => {
        ...
        expect(metrics).toHaveLength(1);
        expect(metrics[0].label).toBe("method_ok");
        await expect(wrapped.createTodo("Bad", "wrong")).rejects.toThrow(UnauthorizedError);
    ...
    it("full pipeline: retry + result bridge (caller) + auth + observability (callee)", async () => {
        ...
        const instance = createAppDO();
        const wrappedInstance = withCalleeTransforms(instance, [
            createObservabilityCallee((m) => metrics.push(m)),
            createAuthGuard({
                protectedMethods: ["createTodo"],
                tokenExtractor: (args) => args[1],
...
describe("better-result bridge (e2e)", () => {
    describe("separate caller + callee codecs", () => {
        it("round-trips a Result.ok through serialize → deserialize", async () => {
            const callee = createBetterResultCalleeCodec();
            const caller = createBetterResultCallerCodec();
            const instance = createAppDO();
    ...
    describe("full bridge (caller().callee())", () => {
        it("round-trips a success Result through the bridge", async () => {
            const bridge = createBetterResultBridge();
            const instance = createAppDO();
    ...
    describe("metricsWithBetterResult (error-type metrics, manual TStub)", () => {
        it("emits rpc_ok for successful Result values", async () => {
            ...
            const stub = withTransforms(mockNamespace(() => wrapped))
                .get("x")
                .with(bridge)
                .with(createMetricsWithBetterResult({
