...
import { createTransform, TRANSFORM_CALL_ID_CONTEXT_KEY, useDOTransforms, withCalleeTransforms, withTransforms, } from "../src";
import { errorBoundary, NetworkError, rateLimiter, singleFlight, TimeoutError, UnknownRpcError, } from "./workers/src/transforms";
function createCounterStub() {
    let count = 0;
    let failureCount = 0;
    return {
        id: "counter-1",
 ...
 */
function mockNamespace(factory) {
    return {
...
describe("withTransforms (caller)", () => {
    it("runs transforms in registration order", async () => {
        const events = [];
        const trace = createTransform().caller(() => async ({ method, next }) => {
    ...
    it("allows caller transforms to override method arguments", async () => {
        function multiplyArg(multiplier) {
            return createTransform().caller(() => ({ args, next }) => {
                const by = args[0];
                return next({ args: [by * multiplier] });
            })();
        }
        const ns = withTransforms(mockNamespace(createCounterStub));
...
describe("withCalleeTransforms (callee)", () => {
    it("runs callee transforms before the actual method", async () => {
        const events = [];
        const logging = createTransform().callee(() => async ({ method, next }) => {
    ...
    it("runs multiple callee transforms in order", async () => {
        const events = [];
        const first = createTransform().callee(() => async ({ method, next }) => {
            events.push(`first:before:${method}`);
            const result = await next();
            events.push(`first:after:${method}`);
            return result;
        })();
        const second = createTransform().callee(() => async ({ method, next }) => {
    ...
    it("callee transforms can override arguments", async () => {
        const doubleArgs = createTransform().callee(() => async ({ args, next }) => {
            const by = args[0];
            return next({ args: [by * 2] });
        })();
        const instance = createCounterDO();
    ...
    it("callee transforms receive state and env from options", async () => {
        let capturedState;
        let capturedEnv;
        const inspector = createTransform().callee(() => async ({ state, env, next }) => {
            capturedState = state;
            capturedEnv = env;
            return next();
        })();
    ...
    it("callee transforms receive the instance reference", async () => {
        ...
        const inspector = createTransform().callee(() => async ({ instance, next }) => {
    ...
    it("callee transforms can short-circuit (skip next)", async () => {
        const blocker = createTransform().callee(() => async () => {
            return -1;
        })();
        const instance = createCounterDO();
...
describe("next({ result }) short-circuit", () => {
    describe("callee-side", () => {
        it("skips the method but downstream transforms still run", async () => {
            const events = [];
            const guard = createTransform().callee(() => async ({ next }) => {
                events.push("guard");
                return next({ result: 42 });
            })();
            const logger = createTransform().callee(() => async ({ next }) => {
        ...
        it("downstream transform can modify the short-circuited result", async () => {
            ...
            const doubler = createTransform().callee(() => async ({ next }) => {
                const result = await next();
                return result * 2;
            })();
            const instance = createCounterDO();
        ...
        it("preserves context through the short-circuit", async () => {
            let capturedCtx;
            const injector = createTransform().callee(() => async ({ next }) => {
                return next({
                    result: "skipped",
                    context: { origin: "injector" },
                });
            })();
            const inspector = createTransform().callee(() => async ({ context, next }) => {
                capturedCtx = { ...context };
                return next();
            })();
            const instance = createCounterDO();
        ...
        it("normal pipeline is unaffected when result is not provided", async () => {
            const passthrough = createTransform().callee(() => async ({ next }) => {
                return next();
            })();
            const instance = createCounterDO();
    ...
    describe("caller-side", () => {
        it("skips the RPC call but downstream transforms still run", async () => {
            ...
            const guard = createTransform().caller(() => async ({ next }) => {
                events.push("guard");
                return next({ result: 99 });
            })();
            const logger = createTransform().caller(() => async ({ next }) => {
                events.push("logger:before");
                const result = await next();
                events.push(`logger:after:${result}`);
                return result;
...
describe("createTransform with callee().caller()", () => {
    ...
    it("callee().caller() and caller().callee() produce same runtime shape", () => {
        const callerFirst = createTransform()
            .caller(() => async ({ next }) => next())
            .callee(() => async ({ next }) => next());
        const calleeFirst = createTransform()
            .callee(() => async ({ next }) => next())
            .caller(() => async ({ next }) => next());
    ...
    it("callee().caller() with parameterized factories", () => {
        const transform = createTransform()
            .callee((prefix) => async ({ next }) => {
            void prefix;
            return next();
        })
            .caller((retries) => async ({ next }) => {
            void retries;
            return next();
        });
    ...
    it("callee-only usage still works after .caller() is available", () => {
        const calleeOnly = createTransform().callee(() => async ({ next }) => next());
...
describe("parameterized callee transform", () => {
    it("creates parameterized callee transforms via plain function", async () => {
        function clamp(max) {
            return createTransform().callee(() => async ({ args, next }) => {
                const by = args[0];
                const clamped = Math.min(by, max);
                return next({ args: [clamped] });
            })();
        }
        const instance = createCounterDO();
...
describe("createTransform split params", () => {
    it("supports different caller and callee parameter sets", async () => {
        ...
        const typed = createTransform()
            ...
            const by = args[0];
            return next({ args: [Math.min(by, max)] });
        });
...
describe("context", () => {
    describe("caller-side", () => {
        it("starts with an empty context", async () => {
            let capturedCtx;
            const inspector = createTransform().caller(() => async ({ context, next }) => {
                capturedCtx = context;
                return next();
            })();
            const ns = withTransforms(mockNamespace(createCounterStub));
            ...
            expect(capturedCtx?.[TRANSFORM_CALL_ID_CONTEXT_KEY]).toEqual(expect.any(String));
        });
        it("passes context through next()", async () => {
            let capturedCtx;
            const injector = createTransform().caller(() => async ({ next }) => {
                return next({ context: { traceId: "abc-123" } });
            })();
            const reader = createTransform().caller(() => async ({ context, next }) => {
                capturedCtx = context;
                return next();
            })();
            const ns = withTransforms(mockNamespace(createCounterStub));
        ...
        it("merges context from multiple transforms", async () => {
            let capturedCtx;
            const addTrace = createTransform().caller(() => async ({ next }) => {
                return next({ context: { traceId: "t1" } });
            })();
            const addAccount = createTransform().caller(() => async ({ next }) => {
        ...
        it("later transforms override earlier keys", async () => {
            let capturedCtx;
            const first = createTransform().caller(() => async ({ next }) => {
                return next({ context: { key: "first" } });
            })();
            const second = createTransform().caller(() => async ({ next }) => {
    ...
    describe("callee-side", () => {
        it("receives context from caller envelope", async () => {
            ...
            const reader = createTransform().callee(() => async ({ context, next }) => {
                capturedCtx = context;
                return next();
            })();
            const instance = createCounterDO();
        ...
        it("callee transforms can add to context", async () => {
            ...
            const calleeInjector = createTransform().callee(() => async ({ next }) => {
                return next({ context: { from2: "callee" } });
            })();
            const calleeReader = createTransform().callee(() => async ({ context, next }) => {
                capturedCtx = context;
                return next();
            })();
            const instance = createCounterDO();
        ...
        it("receives id from the instance", async () => {
            let capturedId;
            const inspector = createTransform().callee(() => async ({ id, next }) => {
                capturedId = id;
                return next();
            })();
            const instance = createCounterDO();
    ...
    describe("round-trip", () => {
        it("full caller ��� callee → method round-trip preserves args and context", async () => {
            let callerSeenCtx;
            let calleeSeenCtx;
            const callerInject = createTransform().caller(() => async ({ next }) => {
                return next({
                    context: { requestId: "req-1", coloId: "DFW" },
                });
            })();
            const callerRead = createTransform().caller(() => async ({ context, next }) => {
                callerSeenCtx = { ...context };
                return next();
            })();
            const calleeRead = createTransform().callee(() => async ({ context, next }) => {
        ...
        it("method receives clean args, not the envelope", async () => {
            ...
            const inject = createTransform().caller(() => async ({ next }) => {
...
describe("inline .with() with bare handler", () => {
    ...
    it("can chain inline handlers with transform objects", async () => {
        const events = [];
        const objectTransform = createTransform().caller(() => async ({ next }) => {
            events.push("object");
            return next();
        })();
        const ns = withTransforms(mockNamespace(createCounterStub));
...
describe("useDOTransforms builder", () => {
    ...
    it("inherits class decorator transforms base-first", async () => {
        const events = [];
        const baseLog = createTransform().callee(() => async ({ method, next }) => {
            events.push(`base:${String(method)}`);
            return next();
        })();
        const derivedLog = createTransform().callee(() => async ({ method, next }) => {
    ...
    it("applies transforms only to the decorated method", async () => {
        ...
        const trackDecorated = createTransform().callee(() => async ({ method, next }) => {
            events.push(String(method));
            return next();
        })();
    ...
    it("pipelines addTransformContext from instance and transforms", async () => {
        let capturedContext;
        const callerContext = createTransform().caller(() => async ({ next }) => {
            return next({ context: { requestId: "req-1" } });
        })();
        class ContextualDO {
            async addTransformContext(context) {
                return {
        ...
        const contextReader = createTransform()
        ...
        const contextTransform = {
            ...contextReader,
            addTransformContext(context) {
                return {
                    fromTransform: `tx-${context.requestId ?? "missing"}`,
                };
            },
        };
        useDOTransforms(ContextualDO).with(contextTransform).done();
    ...
    it("attaches a unique call id in context per caller invocation", async () => {
        const callIds = [];
        const callerMetadata = createTransform().caller(() => async ({ next }) => {
        ...
        const captureCallId = createTransform().callee(() => async ({ context, next }) => {
        ...
        expect(callIds[0]).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i);
    });
    it("preserves generated call id even if a transform tries overriding it", async () => {
        ...
        const overrideAttempt = createTransform().caller(() => async ({ next }) => {
            return next({
                context: {
                    tagged: true,
...
describe("withTransforms (namespace proxy)", () => {
    ...
    it("chains multiple transforms via .with().with()", async () => {
        ...
        const wrapped = withTransforms(ns);
        const events = [];
        const first = createTransform().caller(() => async ({ method, next }) => {
            events.push(`first:${method}`);
            return next();
        })();
        const second = createTransform().caller(() => async ({ method, next }) => {
 ...
 */
const errorMetrics = createTransform().caller((log) => async ({ method, next }) => {
