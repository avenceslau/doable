...
/**
 * Context passed from caller to callee across the wire.
 * The metrics caller injects accountId; the callee reads it.
 */
...
/**
 * A single metric log entry recorded by the callee metrics transform.
 * The callee stores these on the DO instance so the test can retrieve
 * them via `getMetrics()`.
 */
...
export const metrics = createTransform<object, {}, MetricsContext>()
	.caller((accountId: string) => async ({ next }) => {
...
export type SingleFlightMetricEntry = {
	method: string;
	status: "busy";
};
 ...
 */
function isMethodBusyError(error: unknown): boolean {
	...
	if (
		"_tag" in error &&
		(error as { _tag: string })._tag === "MethodBusyError"
	) {
		return true;
	}
	...
	if (
		...
		return /^Method ".+" is already running$/.test(
 ...
 */
function matchKnownError(
	err: unknown
): Result<never, NetworkError | TimeoutError> | undefined {
	const message =
...
export const errorBoundary = createTransform<object>().caller(
...
/**
 * Simulates an external rate-limit service.
 * The test provides this; the transform just calls it.
 */
export type RateLimitService = {
	check(method: string): Promise<{ allowed: boolean }>;
};

export const rateLimiter = createTransform<object>().caller(
	(limitedMethod: string, service: RateLimitService) => {
		return async ({ method, next }) => {
...
/**
 * Type-level function that widens the error channel of Result-returning
 * methods to include MethodBusyError, and leaves non-Result methods
 * unchanged.
 */
...
export const singleFlight = createTransform<
	...
	AddBusyError
>()
	.caller((log: SingleFlightMetricEntry[]) => {
		return async ({ method, next }) => {
